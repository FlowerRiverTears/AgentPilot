...
- RAG：文档导入、解析、切块、检索、引用回传、重排
- 切面：鉴权、日志、审计、限流、追踪、成本统计等横切能力
- 向量：embedding 生成、向量存储、相似度检索，默认落在 PostgreSQL + pgvector
...
2. 生产级 RAG 知识库
   ...
   - 支持分块、向量化、关键词检索、混合检索、重排、引用溯源。
   - 支持每次回答展示证据片段，避免“看起来很强但无法验证”。
...
- Embedding Provider：OpenAI-compatible embedding、本地 bge-m3、Qwen embedding。
...
本项目中的 RAG 不是只做“向量搜索 + 拼 prompt”，而是做成可配置流水线：

1. Data Loader：加载 PDF、网页、Markdown、代码仓库。
2. Parser：抽取正文、标题、表格、代码块、元数据。
3. Chunker：按语义标题、token 长度、代码结构切块。
4. Indexer：生成 embedding，写入 pgvector / Qdrant。
5. Retriever：向量检索、BM25、metadata filter、hybrid search。
6. Reranker：对候选片段重排。
7. Citation Builder：把引用片段绑定到回答。
8. Evaluator：评估召回率、引用质量、答案一致性。
...
- knowledge_bases：知识库。
- documents：文档。
- document_chunks：文档切片，包含 embedding。
- eval_datasets：评测集。
- eval_cases：评测用例。
- eval_runs：评测任务。
- audit_logs：审计日志。
