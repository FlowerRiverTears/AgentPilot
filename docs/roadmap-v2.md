...
- [x] 回答重试。
- [x] 更清晰的错误提示。
- [x] Mermaid.js 图表渲染：智能体回答中的 ` ```mermaid ` 代码块自动渲染为流程图、时序图、甘特图等可视化图表。
...
- [x] 对话记忆管理：查看历史会话列表、删除会话、会话搜索。
- [x] 上下文压缩（滑动窗口 + 摘要）：长对话自动压缩早期上下文，控制 Token 消耗和响应耗时。
- [x] 多知识库绑定：一个智能体绑定多个知识库，检索时合并结果。
- [x] 智能体导入/导出：JSON 格式导出智能体配置，跨环境迁移。
- [x] 回答反馈：用户对回答点赞/点踩，记录到数据库，作为智能体质量评估的数据基础。
- [x] Prompt 模板库：预置常用系统提示词模板，一键套用，降低创建智能体门槛。
