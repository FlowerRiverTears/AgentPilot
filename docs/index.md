...
| 方法 | 端点 | 说明 |
|------|------|------|
| GET | `/tools/calls` | 查询最近的工具调用日志（需鉴权） |

### 5.10 文档管理接口

| 方法 | 端点 | 说明 |
|------|------|------|
| GET | `/knowledge-bases/{kb_id}/documents` | 列出知识库下的文档（需鉴权） |
| DELETE | `/knowledge-bases/{kb_id}/documents/{doc_id}` | 删除单个文档（需鉴权） |

### 5.11 运行重试接口

| 方法 | 端点 | 说明 |
|------|------|------|
| POST | `/runs/{run_id}/retry` | 根据历史 Run 重新执行（需鉴权） |
...
- [x] 文档级管理和单文档删除
- [ ] OCR 支持扫描件 PDF
- [x] 多模态 Embedding（CLIP 等统一向量空间模型）
- [x] 图片提取（PDF 中的图片独立提取、切块和向量化）
- [x] 跨模态检索（文本查图片、图片查文本）
- [x] document_chunks 表增加 content_type 和 image_url 字段
- [x] Embedding 服务增加 embed_image 方法
